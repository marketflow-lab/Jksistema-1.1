        ...
        async function salvarConfigLoja(card) {
            ...
            try {
                ...
                alert(`Erro ao salvar configuração da loja: ${error.message || error}`);
                renderizarLojas();
            }
        }
        ...
        async function carregarLojas() {
            ...
            try {
                ...
                lojasGrid.innerHTML = `<div class="empty-state"><div><h2>Falha ao carregar lojas</h2><p>${escapeHtml(error.message || error)}</p></div></div>`;
            }
        }
        ...
        async function carregarPerguntas(pagina = 1) {
            ...
            try {
                ...
                perguntasStatus.textContent = `Erro ao carregar perguntas: ${error.message || error}`;
                perguntasList.innerHTML = '';
                perguntasPagination.classList.add('hidden');
                perguntasPagination.innerHTML = '';
            } finally {
        ...
        async function abrirConversaPosVenda(venda) {
            ...
            try {
                ...
                posVendaDetail.innerHTML = `<div class="answer-box empty">Erro ao carregar conversa: ${escapeHtml(error.message || error)}</div>`;
            } finally {
                state.posVendaDetalheCarregando = false;
            }
        }
        ...
        async function enviarRespostaPosVenda(conversa, textarea, botao, status) {
            ...
            try {
                ...
                status.textContent = `Erro ao enviar resposta: ${error.message || error}`;
        ...
        async function carregarPosVenda(forcar = false) {
            ...
            try {
                ...
                posVendaStatus.textContent = cacheLocal && cacheLocal.data
                    ? `Erro ao atualizar pela API: ${error.message || error}. Mantendo as informações salvas.`
                    : `Erro ao carregar pós venda: ${error.message || error}`;
        ...
        async function carregarSkusTreinamentoAI() {
            ...
            try {
                ...
                aiTrainingSkuInfo.classList.add('hidden');
                aiTrainingSkuInfo.innerHTML = '';
                aiTrainingStatus.textContent = `Erro ao carregar SKUs: ${error.message || error}`;
            }
        }
        ...
        async function carregarTreinamentoAI() {
            ...
            try {
                ...
                aiTrainingStatus.textContent = `Erro ao carregar orientações: ${error.message || error}`;
            }
        }
        ...
        async function salvarTreinamentoAI() {
            ...
            try {
                ...
                aiTrainingStatus.textContent = `Erro ao salvar orientações: ${error.message || error}`;
                throw error;
            } finally {
        ...
        async function simularTreinamentoAI() {
            ...
            try {
                ...
                respostaChat.setError(`Erro na simulação: ${error.message || error}`);
                aiTrainingStatus.textContent = `Erro na simulação: ${error.message || error}`;
            } finally {
                btnAiTrainingSalvar.disabled = false;
                btnAiTrainingSimular.disabled = false;
            }
