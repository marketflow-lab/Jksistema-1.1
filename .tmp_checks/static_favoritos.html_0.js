        ...
        function iniciarAjusteLarguraSkuSidebar(event) {
            if (!mlSkuSidebarSectionEl) return;
            if (event.button !== undefined && event.button !== 0) return;
            event.preventDefault();
            document.body.classList.add('ml-sidebar-resizing');
        ...
        function aplicarEstadoSkuSidebar() {
            if (mlSkuSidebarSectionEl) {
                mlSkuSidebarSectionEl.classList.remove('is-hidden');
            }
            document.body.classList.remove('ml-sku-sidebar-collapsed');
        }
        ...
        function montarItensSkuSidebarMercadoLivre() {
            ...
            const itens = (mlSkusAnunciosLojaAtual || [])
                .map((item, index) => ({
                    ...
                    index,
                    chave: String(item && item.sku || '').trim().toLowerCase()
                }))
                .filter(item => {
        ...
        function filtrarItensSkuSidebarMercadoLivre(itens = montarItensSkuSidebarMercadoLivre()) {
            const termo = normalizarTextoMl(mlSkuSidebarFiltro || (mlSkuSidebarSearchEl && mlSkuSidebarSearchEl.value) || '');
        ...
        async function enriquecerAnunciosFavoritosRanking(anuncios) {
            ...
            const semComplemento = pendentes.filter(item => item && (!vendedorValido(item.vendedor) || !obterImagemAnuncioFavoritos(item) || precisaComplementoPrecoFavoritos(item) || !obterTipoAnuncioFavoritos(item) || fullAnuncioDesconhecidoFavoritos(item)));
        ...
        async function filtrarAnunciosFavoritosPorIa(info, anuncios) {
            ...
            if (!lista.length) return { anuncios: lista, removidos: [], removidosTotal: 0 };

            try {
                mostrarBalaoFavoritosStatus(`SKU ${info.sku}: validando se os anuncios sao do mesmo produto com IA...`);
                const response = await fetch('/api/favoritos/ranking/filtrar-ia', {
                    ...
                    body: JSON.stringify({
                        ...
                        titulo_sku: info.titulo || '',
                        descricao_sku: info.descricao || '',
                        pesquisas: (info.termos || []).map(item => item.termo).filter(Boolean),
                        max_anuncios: Math.min(220, lista.length),
                        anuncios: lista.map(anuncio => ({
                            id: normalizarIdAnuncioFavoritosIa(anuncio),
                            mlb: normalizarIdAnuncioFavoritosIa(anuncio),
                ...
                const data = await response.json();
                const manterIds = new Set((Array.isArray(data.manter_ids) ? data.manter_ids : [])
                    .map(id => String(id || '').trim().toUpperCase().replace(/-/g, ''))
                    .filter(Boolean));
                const removerIds = new Set((Array.isArray(data.remover_ids) ? data.remover_ids : [])
                    .map(id => String(id || '').trim().toUpperCase().replace(/-/g, ''))
                    .filter(Boolean));
                const removidos = Array.isArray(data.removidos) ? data.removidos : [];
                if (!manterIds.size && !removerIds.size) {
                ...
                const filtrados = lista.filter(anuncio => {
                    ...
                    if (!id) return true;
                    if (manterIds.size) return manterIds.has(id);
                    return !removerIds.has(id);
                });
                return {
                    anuncios: filtrados,
                    removidos,
                    removidosTotal: removerIds.size || Math.max(0, lista.length - filtrados.length)
                };
            } catch (err) {
                if (mlFavoritosCancelado || (err && (err.canceladoFavoritos || err.name === 'AbortError'))) throw err;
                console.warn('Nao foi possivel filtrar ranking por IA:', err);
                mostrarBalaoFavoritosStatus(`SKU ${info.sku}: IA nao conseguiu validar os anuncios; mantendo ranking coletado.`);
                return { anuncios: lista, removidos: [], removidosTotal: 0 };
            }
        }
        ...
        function renderizarHistoricoSkuSidebar() {
            ...
            histMlSkuSidebarCountEl.textContent = itens.length ? `${itens.length} SKU(s)` : '';
            histMlSkuSidebarEmptyEl.textContent = mlSkuLojaSelecionada
            ...
            if (selecionadoChave && !itens.some(item => item.chave === selecionadoChave)) {
                histMlSkuSelecionado = '';
            }
        ...
        function renderizarHistoricoFavoritos() {
            ...
            if (!skuSelecionadoChave) {
                mlHistoricoFavoritosEmptyEl.textContent = 'Selecione um SKU na barra lateral para ver o historico.';
                mlHistoricoFavoritosEmptyEl.classList.remove('hidden');
                if (mlHistoricoFavoritosStatusEl) {
                    mlHistoricoFavoritosStatusEl.textContent = historico.length
                        ? `${historico.length} execucao(oes) salva(s) no servidor${lojaAtualTexto ? ` para a loja ${lojaAtualTexto}` : ''}.`
        ...
        async function fazerFavoritosSkusSelecionados() {
            ...
            try {
                ...
                for (let idx = 0; idx < selecionados.length; idx += 1) {
                    ...
                    verificarCancelamentoFavoritos();
                    const filtroIa = await filtrarAnunciosFavoritosPorIa(info, unicos);
                    verificarCancelamentoFavoritos();
        ...
        function renderizarFavoritosSkuSidebar() {
            ...
            favMlSkuSidebarCountEl.textContent = itens.length ? `${itens.length} SKU(s)` : '';
            favMlSkuSidebarEmptyEl.textContent = mlSkuLojaSelecionada
                ? 'Nenhum SKU encontrado nos anuncios ativos desta loja.'
                : 'Escolha uma loja integrada para carregar os SKUs dos anuncios ativos.';
        ...
        function prepararAbaFavoritosMl() {
            if (!favMlSkuSelecionado) {
                const ultimoSku = obterUltimoSkuHistoricoFavoritos();
                if (ultimoSku) favMlSkuSelecionado = ultimoSku;
            }
            renderizarFavoritosSkuSidebar();
            renderizarFavoritosOutrosAnuncios(favMlSkuSelecionado);
            if (favMlSkuSelecionado) {
