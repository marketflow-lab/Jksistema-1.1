        ...
        if (!verificarSessao()) {
            // verificarSessao redireciona quando nÃ£o hÃ¡ token.
        }
        ...
        function textoGanhoVendasML(segmento) {
            if (!segmento || segmento.sales_percent_increase === null || segmento.sales_percent_increase === undefined) {
                return 'Sem projeÃ§Ã£o de aumento';
            }
            const valor = Number(segmento.sales_percent_increase);
            if (!Number.isFinite(valor)) return 'Sem projeÃ§Ã£o de aumento';
        ...
        function metricTempoRespostaML(titulo, segmento, subtituloPadrao) {
            ...
            const subtitulo = segmento && segmento.sales_percent_increase !== undefined
                ...
                : subtituloPadrao;
            return `<div class="metric"><strong>${escapeHtml(formatarTempoRespostaML(tempo))}</strong><span>${escapeHtml(titulo)} Â· ${escapeHtml(subtitulo)}</span></div>`;
        }
        ...
        function renderizarLojas() {
            if (!state.lojas.length) {
                const vazio = '<div class="empty-state"><div><h2>Nenhuma loja encontrada</h2><p>Cadastre uma loja em IntegraÃ§Ãµes para comeÃ§ar.</p></div></div>';
                lojasGrid.innerHTML = vazio;
                posVendaLojasGrid.innerHTML = vazio;
                perguntasStatus.textContent = 'Nenhuma loja cadastrada em IntegraÃ§Ãµes.';
                    ...
                    <div class="store-card ${active ? 'active' : ''} ${conectado ? '' : 'disabled'}" data-loja="${escapeHtml(nome)}" tabindex="${conectado ? '0' : '-1'}" aria-disabled="${conectado ? 'false' : 'true'}">
                        ...
                        <span class="store-options">
                            ...
                            <label class="store-option">
                                ...
                                <span>Solicitar aprovaÃ§Ã£o antes de mandar para o usuÃ¡rio</span>
                            </label>
                        </span>
                    </div>
                `;
        ...
        async function salvarConfigLoja(card) {
            ...
            try {
                ...
                if (!response.ok) throw new Error(data.detail || 'Erro ao salvar configuraÃ§Ã£o.');
                const loja = state.lojas.find((item) => String(item.nome || '') === nome);
                if (loja) loja.config_perguntas = data.config_perguntas || {};
                lojasStatus.textContent = 'ConfiguraÃ§Ã£o da loja salva.';
                renderizarLojas();
                iniciarAutomacaoPerguntas();
            } catch (error) {
                alert(`Erro ao salvar configuraÃ§Ã£o da loja: ${mensagemErro(error)}`);
                renderizarLojas();
            }
        }
        ...
        async function carregarAprovacoesPendentes() {
            try {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(data.detail || 'Erro ao carregar aprovaÃ§Ãµes pendentes.');
                (Array.isArray(data.pendentes) ? data.pendentes : []).forEach(notificarAprovacaoSidebar);
            } catch (error) {
                console.warn('[Perguntas IA] NÃ£o foi possÃ­vel carregar aprovaÃ§Ãµes pendentes:', error);
            }
        }
        ...
        async function executarAutomacaoPerguntas() {
            ...
            try {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(data.detail || 'Erro na automaÃ§Ã£o de perguntas.');
                ...
                console.warn('[Perguntas IA] Falha ao executar automaÃ§Ã£o:', error);
            } finally {
        ...
        async function executarAutomacaoPosVenda() {
            ...
            try {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(data.detail || 'Erro na automaÃ§Ã£o de pÃ³s venda.');
                ...
                console.warn('[PÃ³s venda IA] Falha ao executar automaÃ§Ã£o:', error);
            } finally {
        ...
        function renderizarResumo(data) {
            ...
            const metricasTempo = tempoML.available ? [
                metricTempoRespostaML('Tempo mÃ©dio ML', tempoML.total || {}, 'Ãšltimos 14 dias'),
                metricTempoRespostaML('HorÃ¡rio comercial', tempoML.weekdays_working_hours || {}, 'Seg. a sex. 9h-18h'),
                metricTempoRespostaML('Fora do horÃ¡rio', tempoML.weekdays_extra_hours || {}, 'Seg. a sex. 18h-00h'),
                metricTempoRespostaML('Fim de semana', tempoML.weekend || {}, 'SÃ¡b. e dom.')
            ].join('') : `
                <div class="metric"><strong>Sem dados</strong><span>Tempo de resposta ML Â· ${escapeHtml(tempoML.erro || 'MÃ©trica indisponÃ­vel')}</span></div>
            `;
            ...
            perguntasSummary.innerHTML = `
                ...
                <div class="metric"><strong>${pendentes}</strong><span>NÃ£o respondidas</span></div>
        ...
        function renderizarPaginacaoPerguntas(total) {
            ...
            perguntasPagination.innerHTML = `
                ...
                <div class="pagination-actions">
                    ...
                    <span>PÃ¡gina ${state.paginaPerguntas} de ${totalPaginas}</span>
                    <button class="pagination-btn" type="button" data-page="${state.paginaPerguntas + 1}" ${state.paginaPerguntas >= totalPaginas ? 'disabled' : ''}>PrÃ³xima</button>
                </div>
            `;
        ...
        function renderizarPerguntas() {
            ...
            if (!perguntas.length) {
                perguntasList.innerHTML = '<div class="empty-state"><div><h2>Nenhuma pergunta encontrada</h2><p>NÃ£o hÃ¡ perguntas para o filtro selecionado nesta loja.</p></div></div>';
                perguntasPagination.classList.add('hidden');
                perguntasPagination.innerHTML = '';
                return;
            }
            ...
            perguntasList.innerHTML = pagina.map((pergunta) => {
                const titulo = pergunta.item_title || pergunta.item_id || 'AnÃºncio';
                                ...
                                <div class="question-top">
                                    <div>
                                        ...
                                        <div class="question-meta">
                                            <span>AnÃºncio ${escapeHtml(pergunta.item_id || '-')}</span>
        ...
        async function carregarPerguntas(pagina = 1) {
            ...
            try {
                ...
                perguntasStatus.textContent = data.interrompido
                    ? `${perguntas.length} pergunta(s) carregada(s). Mostrando 20 por pÃ¡gina, das mais recentes para as mais antigas. O Mercado Livre limitou esta busca aos registros mais recentes.`
                    : `${perguntas.length} pergunta(s) carregada(s) da loja ${state.lojaSelecionada}. Mostrando 20 por pÃ¡gina, das mais recentes para as mais antigas.`;
        ...
        function salvarCachePosVenda(data) {
            try {
                ...
                console.warn('[PÃ³s venda] NÃ£o foi possÃ­vel salvar cache:', error);
            }
        }
        ...
        function renderizarResumoPosVenda(data) {
            ...
            posVendaSummary.innerHTML = `
                ...
                <div class="metric"><strong>${Number(data.orders_total || 0)}</strong><span>Vendas no perÃ­odo</span></div>
        ...
        function renderizarPaginacaoPosVenda(data, conversas) {
            ...
            posVendaPagination.innerHTML = `
                <span>PÃ¡gina ${state.posVendaPagina} Â· ${conversas.length} venda(s) com conversa</span>
                <div class="pagination-actions">
                    <button class="pagination-btn" type="button" data-action="prev" ${temAnterior ? '' : 'disabled'}>Anterior</button>
                    <button class="pagination-btn" type="button" data-action="next" ${temProxima ? '' : 'disabled'}>PrÃ³xima</button>
                </div>
            `;
        ...
        function renderizarPosVenda(conversas) {
            if (!conversas.length) {
                ...
                posVendaDetail.classList.add('hidden');
                posVendaDetail.innerHTML = '';
                posVendaList.innerHTML = '<div class="empty-state"><div><h2>Nenhuma conversa encontrada</h2><p>NÃ£o hÃ¡ vendas com conversa iniciada no perÃ­odo selecionado.</p></div></div>';
                return;
            }
            ...
            posVendaList.innerHTML = conversas.map((venda, index) => {
                ...
                const itensTexto = itens.length
                    ? itens.map((item) => {
                        ...
                        const sku = item.sku ? ` Â· SKU ${escapeHtml(item.sku)}` : '';
                ...
                return `
                    <article class="question-card conversation-card ${active ? 'active' : ''}" data-index="${index}">
                        ...
                        <div class="answer-box ${venda.last_message_text ? '' : 'empty'}">${escapeHtml(venda.last_message_text || 'Conversa iniciada sem prÃ©via disponÃ­vel.')}</div>
                            </div>
                        </div>
                    </article>
                `;
            ...
            const mensagensHtml = mensagens.length
                ? mensagens.map((msg) => `
                    <div class="pos-sale-message ${msg.from_role === 'seller' ? 'seller' : 'buyer'}">
                        <span class="training-message-role">${msg.from_role === 'seller' ? 'Vendedor' : 'Comprador'} Â· ${escapeHtml(formatarData(msg.date))}</span>
                        ${escapeHtml(msg.text || '-')}
                    </div>
                `).join('')
        ...
        async function abrirConversaPosVenda(venda) {
            ...
            posVendaDetail.classList.remove('hidden');
            posVendaDetail.innerHTML = '<div class="status-line">Carregando histÃ³rico completo da conversa...</div>';
        ...
        function renderizarDadosPosVenda(data, origemCache = false) {
            ...
            posVendaStatus.textContent = origemCache
                ? `Exibindo informaÃ§Ãµes salvas de ${state.lojaSelecionada}. Atualizando pela API...`
                : (data.interrompido
                    ? `${conversas.length} venda(s) com conversa. A consulta foi limitada Ã s vendas mais recentes.`
        ...
        async function carregarPosVenda(forcar = false) {
            ...
            if (cacheLocal && cacheLocal.data) {
                ...
                posVendaPagination.classList.add('hidden');
                posVendaPagination.innerHTML = '';
                posVendaStatus.textContent = `Carregando pÃ¡gina ${state.posVendaPagina} de vendas com conversa de ${state.lojaSelecionada}...`;
            }
            ...
            try {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(data.detail || 'Erro ao carregar pÃ³s venda.');
                ...
                posVendaStatus.textContent = cacheLocal && cacheLocal.data
                    ? `Erro ao atualizar pela API: ${mensagemErro(error)}. Mantendo as informaÃ§Ãµes salvas.`
                    : `Erro ao carregar pÃ³s venda: ${mensagemErro(error)}`;
        ...
        function renderizarSkuTreinamentoInfo() {
            ...
            aiTrainingSkuInfo.innerHTML = `
                ...
                <div>
                    <div class="training-sku-name">SKU ${escapeHtml(sku)} Â· ${escapeHtml(nome)}</div>
                    <div class="training-sku-meta">${escapeHtml(meta || 'Dados do cadastro serÃ£o enviados junto com a pergunta.')}</div>
                </div>
            `;
        }
        ...
        function renderizarNotasSkuTreinamento() {
            ...
            aiTrainingNotasSku.placeholder = sku
                ? 'AplicaÃ§Ãµes confirmadas, cÃ³digos, variaÃ§Ãµes, exceÃ§Ãµes e cuidados para este SKU.'
                : 'Selecione um SKU para salvar notas especÃ­ficas.';
        }
        ...
        function renderizarExemplosTreinamento() {
            ...
            aiTrainingExamplesList.innerHTML = exemplos.map((item, index) => `
                <div class="training-example-item">
                    <strong>${escapeHtml(item.sku ? `SKU ${formatarSkuExibicao(item.sku)}` : `Exemplo ${index + 1}`)}</strong>
        ...
        function adicionarExemploTreinamento() {
            ...
            if (!pergunta || !resposta) {
                aiTrainingStatus.textContent = 'Informe pergunta e resposta para adicionar um exemplo.';
                return;
            }
            const tipo = state.treinamentoTipo === 'pos_venda' ? 'pos_venda' : 'perguntas_anuncio';
            const exemplos = normalizarExemplosTreinamento((state.treinamentoDados[tipo] || {}).exemplos || []);
            exemplos.unshift({
                pergunta,
                resposta,
                sku: String(aiTrainingSku.value || '').trim(),
                observacao: '',
            ...
            renderizarExemplosTreinamento();
            aiTrainingStatus.textContent = 'Exemplo adicionado. Clique em salvar para manter no servidor.';
        }
        ...
        function rotuloTipoTreinamento(tipo = state.treinamentoTipo) {
            return tipo === 'pos_venda' ? 'pÃ³s-venda' : 'perguntas de anÃºncio';
        }
        ...
        function atualizarStatusTreinamentoTipo() {
            ...
            if (dados.updated_at) {
                aiTrainingStatus.textContent = `OrientaÃ§Ãµes de ${rotulo} salvas em ${formatarData(dados.updated_at)}`;
            } else if (dados.orientacoes) {
                aiTrainingStatus.textContent = `OrientaÃ§Ãµes de ${rotulo} carregadas.`;
            } else {
                aiTrainingStatus.textContent = `Nenhuma orientaÃ§Ã£o de ${rotulo} salva ainda.`;
            }
        }
        ...
        function renderizarTipoTreinamento(limparChat = false) {
            ...
            aiTrainingOrientacoesLabel.textContent = tipo === 'pos_venda'
                ? 'OrientaÃ§Ãµes para pÃ³s-venda'
                : 'OrientaÃ§Ãµes para perguntas de anÃºncio';
            aiTrainingOrientacoes.placeholder = tipo === 'pos_venda'
                ? 'Ex.: agradecer a compra, pedir fotos ou vÃ­deo quando necessÃ¡rio, orientar garantia e troca sem prometer aprovaÃ§Ã£o...'
                : 'Ex.: responder sem Markdown, nÃ£o prometer prazo sem confirmaÃ§Ã£o, pedir modelo/ano quando a aplicaÃ§Ã£o do produto for incerta...';
            if (aiTrainingChatHead) {
                aiTrainingChatHead.textContent = tipo === 'pos_venda'
                    ? 'SimulaÃ§Ã£o de pÃ³s-venda'
                    : 'SimulaÃ§Ã£o de pergunta de anÃºncio';
            }
        ...
        async function carregarTreinamentoAI() {
            ...
            aiTrainingStatus.textContent = 'Carregando orientaÃ§Ãµes...';
            try {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(data.detail || 'Erro ao carregar orientaÃ§Ãµes.');
                ...
                aiTrainingStatus.textContent = `Erro ao carregar orientaÃ§Ãµes: ${mensagemErro(error)}`;
            }
        }
        ...
        async function salvarTreinamentoAI() {
            ...
            aiTrainingStatus.textContent = 'Salvando orientaÃ§Ãµes...';
            try {
                const response = await fetch('/api/mercadolivre/ia-treinamento', {
                    method: 'POST',
                    headers: {
                ...
                const data = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(data.detail || 'Erro ao salvar orientaÃ§Ãµes.');
                ...
                aiTrainingStatus.textContent = `Erro ao salvar orientaÃ§Ãµes: ${mensagemErro(error)}`;
                throw error;
            } finally {
        ...
        async function simularTreinamentoAI() {
            ...
            try {
                ...
                btnAiTrainingSalvar.disabled = true;
                btnAiTrainingSimular.disabled = true;
                aiTrainingStatus.textContent = 'Gerando resposta de simulaÃ§Ã£o...';
                ...
                aiTrainingStatus.textContent = 'SimulaÃ§Ã£o gerada com as orientaÃ§Ãµes salvas.';
            } catch (error) {
                respostaChat.setError(`Erro na simulaÃ§Ã£o: ${mensagemErro(error)}`);
                aiTrainingStatus.textContent = `Erro na simulaÃ§Ã£o: ${mensagemErro(error)}`;
            } finally {
                btnAiTrainingSalvar.disabled = false;
                btnAiTrainingSimular.disabled = false;
            }
