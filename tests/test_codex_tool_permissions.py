...
class CodexToolPermissionsTest(unittest.TestCase):
    def test_every_registered_tool_has_an_explicit_permission_policy(self):
        registered = {str(tool.get("id") or "") for tool in assistant_catalog_data.CODEX_DATA_TOOLS}
        covered = set(assistant_catalog.ASSISTANT_TOOL_PERMISSION_REQUIREMENTS)
        covered.update(assistant_catalog.ASSISTANT_FULL_ONLY_TOOLS)
        self.assertEqual(registered, covered)

    def test_catalog_only_contains_tools_allowed_for_user(self):
        tools = assistant_catalog._assistant_tools_public({"vendas": True})
    ...
    def test_catalog_is_fail_closed_without_permissions(self):
        self.assertEqual(assistant_catalog._assistant_tools_public(), [])
        self.assertEqual(console_prompt_context._codex_agent_tool_catalog({}), [])

    def test_full_catalog_keeps_all_registered_tools(self):
        tools = assistant_catalog._assistant_tools_public({"full": True})
        self.assertEqual(len(tools), len(assistant_catalog_data.CODEX_DATA_TOOLS))

    def test_non_full_prompt_does_not_read_shared_operational_memory(self):
        with patch.object(
            console_agent_prompt.codex_operational_memory,
            "compact_context",
    ...
    def test_legacy_context_is_disabled_for_non_full_user(self):
        with patch.object(
            ...
            result = console_prompt_context._codex_app_data_context(
                "liste vendas",
                {},
                "tenant-test",
                {"vendas": True},
    ...
    def test_executor_runs_allowed_tool_but_skips_forbidden_fallbacks(self):
        ...
        with patch.object(assistant_execution, "_assistant_execute_registry_tool", side_effect=fake_execute):
            result = assistant_execution.execute_tool_call(
                client_id="tenant-test",
        ...
        self.assertEqual((result.get("evidence") or {}).get("next_sources"), [])
