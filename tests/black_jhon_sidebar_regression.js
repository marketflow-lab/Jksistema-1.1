...
assert.strictEqual((ui.match(/id="jk-questions-fab"/g) || []).length, 1, 'deve existir um unico atalho de perguntas ML');
...
assert.match(ui, /id="jk-questions-panel"/);
assert.match(ui, /Perguntas Mercado Livre/);
assert.match(ui, /jk-questions-bubble/);
assert.strictEqual((ui.match(/\/assets\/mercado-livre-symbol\.svg\?v=20260815-official-v1/g) || []).length, 2);
assert.doesNotMatch(ui, /jk-questions-logo-bg|jk-questions-handshake/);
assert.match(mercadoLivreSymbol, /viewBox="0 0 16\.933 16\.933"/);
assert.match(mercadoLivreSymbol, /fill="#ffd100"/);
assert.match(mercadoLivreSymbol, /class="cls-1"/);
...
assert.match(codex, /authentication_state/);
assert.match(codex, /authentication_check_pending/);
assert.match(codex, /codexAuthStatusPollTimer = setTimeout/);
assert.match(codex, /codexAuthStatusPollDeadline = now \+ 10000/);
assert.match(codex, /requestGeneration !== codexStatusRequestGeneration/);
assert.match(codex, /codexStatusAbortController\.abort\(\)/);
assert.match(codex, /signal: statusController \? statusController\.signal : undefined/);
assert.match(bootstrap, /if \(codexStatusAbortController\) codexStatusAbortController\.abort\(\)/);
assert.match(codex, /if \(codexPanelAberto\) void _codexCarregarStatus\(true, true\)/);
...
assert.doesNotMatch(codex, /summary=true&channel=unified/);
...
assert.match(codex, /function _codexEhRegistroPerguntaMl/);
assert.match(codex, /function _codexRemoverPerguntasMlDoHistoricoLocal/);
assert.doesNotMatch(codex, /approval_payload/);
...
assert.doesNotMatch(codex, /CODEX_COORDINATOR_FIRST_DELAY_MS|CODEX_COORDINATOR_RETRY_MS|CODEX_COORDINATOR_IDLE_MS|CODEX_PROACTIVE_INTERVAL_MS/);
...
assert.doesNotMatch(codex, /_codexExecutarCoordenador|_codexIniciarAssistenteProativo|_codexRodarProativo|_codexRodarAnaliseSemanal/);
assert.doesNotMatch(codex, /\/api\/admin\/codex\/assistant\/(?:suggestions|proactive\/run|daily-analysis\/run|weekly-analysis\/run)/);
assert.match(codex, /function _codexEhAlertaAutomatico\(item\)/);
assert.match(codex, /function _codexRemoverAlertasAutomaticosDoHistoricoLocal\(\)/);
assert.match(codex, /filter\(item => !_codexEhAlertaAutomatico\(item\)\)/);
...
assert.match(bootstrap, /function toggleQuestionsPanel/);
...
assert.doesNotMatch(bootstrap, /_codexIniciarAssistenteProativo|_codexPararAssistenteProativo/);
assert.match(bootstrap, /_codexRemoverAlertasAutomaticosDoHistoricoLocal\(\)/);
assert.match(bootstrap, /codexFullTextCache\.clear\(\)/);
assert.doesNotMatch(bootstrap, /jk-codex-fab/);

assert.match(approvals, /function _questionsCarregarPendentes/);
assert.match(approvals, /jk-questions-list/);
assert.match(approvals, /_questionsUpsert/);
assert.doesNotMatch(approvals, /_approvalRenderNoPainelBlackJhon|_approvalPersistirNoHistoricoBlackJhon|jk-codex-messages|black-jhon-approval/);
...
assert.match(monitor, /_questionsCarregarPendentes\(\{ silencioso: true, notificarNovas: true \}\)/);
