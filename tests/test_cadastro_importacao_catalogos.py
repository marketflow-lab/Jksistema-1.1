...
def test_preview_ml_planeja_capa_confiavel_sem_expor_dados_da_imagem():
    ...
    assert result["can_apply"] is True
    assert result["summary"]["campos"] == 4
...
def test_preview_ml_preserva_foto_local_existente():
    ...
    assert all(
        change["field"] != "foto" for change in result["items"][0]["changes"]
    )


def test_preview_ml_sem_foto_local_continua_aplicavel_quando_url_ja_estava_salva():
    ...
    assert result["can_apply"] is True
    assert result["items"][0]["status"] == "preencher"
...
def test_preview_ml_aplica_sku_confirmado_e_ignora_anuncio_sem_sku():
    preview = catalogos._construir_preview(
        "mercadolivre",
        {
            "coverage_complete": False,
            ...
            "items": [
                {
                    "sku": "SKU-ML-VALIDO",
                    "fields": {
                        "descricao": "Descricao confirmada",
                        "mlb_principal": "MLB610",
                        "foto_url_ml": "https://http2.mlstatic.com/D_NQ_NP_MLB610.jpg",
                    },
                }
            ],
            "skipped": [{"reason": "sku_missing", "mlb": "MLB611"}],
            "warnings": ["enriquecimento_opcional_incompleto"],
        },
        [],
    )
    ...
    assert preview["summary"]["ignorados"] == 1
    assert preview["apply_rows"][0]["fields"]["descricao"] == "Descricao confirmada"
    assert preview["apply_rows"][0]["photo_plan"] == {
        "url": "https://http2.mlstatic.com/D_NQ_NP_MLB610.jpg",
        "item_id": "MLB610",
    }
...
def test_apply_ml_baixa_capa_e_entrega_foto_ao_writer_atomico(monkeypatch):
    ...
    downloads = []
    source_data_url, _source_bytes = _test_image_data_url(size=(1200, 900))

    def download(url, item_id, deadline):
        downloads.append((url, item_id, deadline))
        return {
    ...
    assert [(url, item_id) for url, item_id, _deadline in downloads] == [
        (photo_url, "MLB123")
    ]
    assert saved[0][0:2] == ("cliente-a", "store-a")
    assert len(saved[0][2]) == 1
    saved_payload = saved[0][2][0]
    assert {
        key: saved_payload[key]
        for key in (
            "sku",
            "row_version",
            "__expected_scope",
    ...
    assert saved_payload["__foto_filename"] == "MLB123.jpg"
    assert saved_payload["__foto_data_url"].startswith("data:image/jpeg;base64,")
    compressed_bytes = base64.b64decode(
        saved_payload["__foto_data_url"].split(",", 1)[1]
    )
...
def test_apply_ml_falha_de_download_nao_impede_salvar_metadados(monkeypatch):
    ...
    def salvar(_client_id, _store_id, payloads, **_kwargs):
...
def test_preparacao_ml_respeita_limite_agregado_e_mantem_lote_aplicavel(
    monkeypatch,
):
    data_url, _source_bytes = _test_image_data_url()
    monkeypatch.setattr(
        catalogos,
    ...
    monkeypatch.setattr(
        catalogos.cadastro_ml,
        "_download_photo_data_url",
        lambda *_args: {"data_url": data_url, "filename": "MLB111.png"},
    )
    saved = []
    monkeypatch.setattr(
        catalogos,
        ...
        [
            {
                ...
                "__catalog_photo_plan": {
                    "url": "https://http2.mlstatic.com/limite.png",
                    "item_id": "MLB111",
                },
            }
        ],
    ...
    assert saved == [{"sku": "LIMITE-1"}]
    assert result["fotos_salvas"] == 0
    assert result["fotos_ignoradas"] == 1


def test_compressao_ml_reduz_dimensoes_tamanho_e_qualidade():
    data_url, source_bytes = _test_image_data_url(
        size=(1400, 1000),
        noisy=True,
    )

    result = catalogos._comprimir_foto_catalogo_ml(
        {"data_url": data_url, "filename": "MLB222.png"},
        "MLB222",
    )

    compressed_bytes = base64.b64decode(result["data_url"].split(",", 1)[1])
    assert result["filename"] == "MLB222.jpg"
