...
def test_lista_compra_salva_quantidades_editadas(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sugestoes, "_recalcular_frete_internacional_itens_lista", lambda _client_id, itens: itens)
...
def test_lista_sugestao_aplica_edicoes_antes_de_gerar_excel(monkeypatch):
    async def fake_visao(*, meses, loja, client_id):
        assert meses == 6
    ...
    def fake_gerar_excel(_nome, itens, *, client_id):
        assert client_id == "000002"
