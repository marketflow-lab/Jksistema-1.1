...
const harnessPath = path.join(__dirname, 'helpers', 'favoritos_workers_pool_electron_harness.js');
...
const env = {
  ...
  JK_APP_URL: 'http://127.0.0.1:9/test-only',
...
try {
  ...
  assert.match(result.stdout || '', /FAVORITOS_WORKERS_POOL_ELECTRON_OK/);
} finally {
