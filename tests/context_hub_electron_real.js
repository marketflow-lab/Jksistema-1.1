...
const harnessPath = path.join(__dirname, 'helpers', 'context_hub_electron_real_harness.js');
const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jk-context-hub-electron-'));
const env = {
  ...process.env,
...
try {
  ...
  assert.match(result.stdout || '', /CONTEXT_HUB_ELECTRON_REAL_OK/);
} finally {
  const resolved = path.resolve(userDataDir);
  const tempRoot = path.resolve(os.tmpdir());
  if (
    process.env.JK_KEEP_ELECTRON_TEST_DATA !== '1'
    && resolved.startsWith(`${tempRoot}${path.sep}`)
    && path.basename(resolved).startsWith('jk-context-hub-electron-')
  ) {
