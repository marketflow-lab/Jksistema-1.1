...
def test_listing_resolves_exact_variation_sku_without_exposing_parent_as_match(monkeypatch):
    _configure_store(monkeypatch, ["Uai Mineirinho"])
    calls = []
    ...
    def fake_api(_client_id, _store, cfg, method, url, **kwargs):
        ...
        calls.append({"method": method, "url": url, "params": params})
        if url.endswith("/items/search"):
            assert params["seller_sku"] == "299-2"
            return FakeResponse(200, {"paging": {"total": 1}, "results": ["MLB2685352638"]}), cfg
        if url.endswith("/items"):
            assert params["include_attributes"] == "all"
            return FakeResponse(200, [{
                "code": 200,
                "body": {
                    "id": "MLB2685352638",
                    ...
                    "variations": [
                        {
                            ...
                            "available_quantity": 32,
                            "sold_quantity": 365,
                            "attribute_combinations": [{"name": "Modelo", "value_name": "Modelo B"}],
                        },
                        {
                            "id": 192168851625,
                            "price": 249.24,
                            ...
                            "sold_quantity": 0,
                            "attribute_combinations": [{"name": "Modelo", "value_name": "Modelo A"}],
                        },
                    ],
                },
            }]), cfg
        if url.endswith("/items/MLB2685352638/variations"):
            assert params == {"include_attributes": "all"}
            return FakeResponse(200, [
                {
                    "id": 175135104424,
                    "attributes": [{"id": "SELLER_SKU", "name": "SKU", "value_name": "299-1"}],
                },
                {
                    "id": 192168851625,
                    "attributes": [{"id": "SELLER_SKU", "name": "SKU", "value_name": "299-2"}],
                },
            ]), cfg
        raise AssertionError(f"URL inesperada: {url}")
    ...
    assert result["found"] is True
    assert result["sku_validation"]["verified_matches"] == 1
    assert match["seller_sku"] == "299-2"
    assert match["parent_sku"] == "1935"
    assert match["matched_sku"] == "299-2"
    assert match["match"] == {
        "exact": True,
        "matched_by": "variation_seller_sku",
        "variation_id": "192168851625",
    }
    assert match["selected_variation"]["sku"] == "299-2"
    assert match["selected_variation"]["option_label"] == "Modelo: Modelo A"
    assert match["available_quantity"] == 0
    assert match["item_available_quantity"] == 32
    assert match["sold_quantity_scope"] == "acumulado_da_variacao"
    assert [variation["sku"] for variation in match["variations"]] == ["299-1", "299-2"]
    assert any("variations?include_attributes=all" in source["resource"] for source in result["sources"])


def test_listing_discards_search_result_when_detailed_variations_do_not_confirm_sku(monkeypatch):
    _configure_store(monkeypatch)

    def fake_api(_client_id, _store, cfg, method, url, **kwargs):
        if url.endswith("/items/search"):
            return FakeResponse(200, {"paging": {"total": 1}, "results": ["MLB100"]}), cfg
        if url.endswith("/items"):
            return FakeResponse(200, [{
                "code": 200,
                "body": {
                    ...
                    "status": "active",
                    "seller_custom_field": "1935",
                    "variations": [{"id": 10, "available_quantity": 5}],
                },
            }]), cfg
        if url.endswith("/items/MLB100/variations"):
            return FakeResponse(200, [{
                "id": 10,
                "attributes": [{"id": "SELLER_SKU", "name": "SKU", "value_name": "299-1"}],
            }]), cfg
    ...
    assert result["sku_validation"]["verified_matches"] == 0
    assert result["sku_validation"]["rejected_ids"] == ["MLB100"]
    assert result["sku_validation"]["complete"] is True
    assert result["coverage_complete"] is True
    assert any("nao confirmaram o SKU 299-2" in warning for warning in result["warnings"])
...
def test_latest_sale_timeout_keeps_exact_coverage_inconclusive(monkeypatch):
    ...
    assert result["target"]["sku"] == "200"
    assert result["exact_coverage"]["complete"] is False
    assert result["exact_coverage"]["stop_reason"] == "timeout"


@pytest.mark.parametrize("status_code", [403, 429, 500])
def test_latest_sale_http_failure_never_becomes_authoritative_empty(monkeypatch, status_code):
    _configure_store(monkeypatch)
    ...
    assert result["error"]
    assert result["exact_coverage"]["complete"] is False
    assert result["exact_coverage"]["stop_reason"] == "target_not_resolved"
...
def test_latest_sale_resolves_sku_and_searches_item_until_exact_variation(monkeypatch):
    _configure_store(monkeypatch)
    calls = []

    def sale_order(order_id: int, sku: str):
        row = _order(order_id)
        row["order_items"][0]["item"]["id"] = "MLB200"
        row["order_items"][0]["item"]["seller_sku"] = sku
        row["date_created"] = f"2026-07-10T10:{order_id % 60:02d}:00.000-03:00"
        return row
    ...
    def fake_api(_client_id, _store, cfg, method, url, **kwargs):
        ...
        calls.append({"method": method, "url": url, "params": params})
        if url.endswith("/items/search"):
            rows = ["MLB200"] if "seller_sku" in params else []
            return FakeResponse(200, {"paging": {"total": len(rows)}, "results": rows}), cfg
        if url.endswith("/orders/search"):
            assert params["q"] == "MLB200"
            offset = int(params.get("offset") or 0)
            if offset < 100:
                rows = [sale_order(offset + index + 1, "OUTRA-VARIACAO") for index in range(50)]
            else:
                rows = [sale_order(65657, "200")]
    ...
    order_calls = [call for call in calls if call["url"].endswith("/orders/search")]
    item_calls = [call for call in calls if call["url"].endswith("/items/search")]
    assert [set(call["params"]).intersection({"seller_sku", "sku"}) for call in item_calls] == [
        {"seller_sku"},
        {"sku"},
    ]
    assert all("status" not in call["params"] for call in item_calls)
    assert len(order_calls) == 3
    assert response["arguments"]["limit"] == 1
    assert result["orders"][0]["order_id"] == "65657"
    assert result["match"]["exact"] is True
    assert result["match"]["matched_by"] == "sku"
    assert result["coverage_complete"] is True
    assert result["exact_coverage"]["orders_inspected"] == 101


def test_latest_return_filters_order_before_fetching_claim_and_return_details(monkeypatch):
    _configure_store(monkeypatch)
    calls = []
    claims = [
        {
            ...
            "resource": "order",
            "type": "return",
            "last_updated": "2026-07-14T11:00:00.000-03:00",
        },
        {
            "id": 7002,
            "resource_id": 222,
            "resource": "order",
            "type": "return",
            "last_updated": "2026-07-14T10:00:00.000-03:00",
        },
    ]
    ...
    def fake_api(_client_id, _store, cfg, method, url, **kwargs):
        ...
        if url.endswith("/items/search"):
            rows = ["MLB200"] if "seller_sku" in params else []
            return FakeResponse(200, {"paging": {"total": len(rows)}, "results": rows}), cfg
        if url.endswith("/claims/search"):
            return FakeResponse(200, {"paging": {"total": 2}, "data": claims}), cfg
        if url.endswith("/orders/111"):
            row = _order(111)
            row["order_items"][0]["item"].update({"id": "MLB999", "seller_sku": "999"})
            return FakeResponse(200, row), cfg
        if url.endswith("/orders/222"):
            row = _order(222)
            row["order_items"][0]["item"].update({"id": "MLB200", "seller_sku": "200"})
            return FakeResponse(200, row), cfg
        if url.endswith("/post-purchase/v2/claims/7002/returns"):
    ...
    assert response["arguments"]["limit"] == 1
    assert response["arguments"]["offset"] == 0
    assert urls.index(next(url for url in urls if url.endswith("/orders/111"))) < urls.index(
        next(url for url in urls if url.endswith("/orders/222"))
    )
    assert not any(url.endswith("/post-purchase/v2/claims/7001/returns") for url in urls)
    assert not any(url.endswith("/post-purchase/v1/claims/7001") for url in urls)
    assert result["devolucoes"][0]["order_id"] == "222"
    assert result["devolucoes"][0]["event_at"] == "2026-07-14T10:05:00.000-03:00"
    assert result["match"]["exact"] is True
    assert result["exact_coverage"]["claims_scanned"] == 2
    assert result["exact_coverage"]["orders_inspected"] == 2
    assert result["coverage_complete"] is True


def test_latest_return_empty_is_authoritative_only_after_complete_scan(monkeypatch):
    _configure_store(monkeypatch)
    ...
    assert result["devolucoes"] == []
    assert result["coverage_complete"] is True
    assert result["exact_coverage"]["stop_reason"] == "exhausted"
