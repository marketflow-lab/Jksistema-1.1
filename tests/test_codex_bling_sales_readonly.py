...
def test_bling_stock_balances_stops_after_first_auth_failure():
    with (
        patch.object(codex_bling_tools, "_connected_stores", return_value=([("Deckas", {"access_token": "expired"})], [])),
        patch.object(
            codex_bling_tools,
            "_call_store",
            return_value=({"error": "Token Bling expirado para esta loja."}, 401, {}),
        ) as call_store,
    ):
        raw = codex_bling_tools.tool_bling_stock_balances(
            "000002", "estoque SKU 001", "Deckas", limit=10
        )

    assert call_store.call_count == 1
    assert raw["result"]["saldos"] == []
    assert len(raw["result"]["sources"]) == 1
    source = raw["result"]["sources"][0]
    assert source["loja"] == "Deckas"
    assert source["path"] == "/produtos"
    assert source["method"] == "GET"
    assert source["records"] == 0
    assert source["status"] == 401
    assert source["params"] == {"produto_search": {}}
    assert any("Token Bling expirado" in warning for warning in raw["result"]["warnings"])
