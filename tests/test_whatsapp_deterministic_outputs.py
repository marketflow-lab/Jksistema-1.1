...
def test_stock_response_deduplicates_identical_fallback_rows():
    row = {
        "loja": "loja selecionada",
        "sku": "200",
        "produto": "Display LCD",
        "saldo_bruto_retornado": 0,
        "saldo_loja_total": None,
        "depositos": [],
        "depositos_excluidos": [],
    }
    ...
    assert response.count("o SKU **200**") == 1
    assert response.count("Saldo bruto retornado pela Bling") == 1
...
def test_whatsapp_simple_ml_order_query_is_not_promoted_to_report_by_style_prompt():
    ...
    assert args == {
        "loja": "JK Pecas",
        "limite": 20,
        "message": "Me mostre os detalhes da ultima venda na JK Pecas",
    }
