...
from backend.services import admin_usuarios_common, codex_actions, codex_assistant, codex_capabilities, codex_console, perguntas_pos_venda_endpoints, perguntas_pos_venda_state, whatsapp_bridge, whatsapp_transcribe
...
def test_question_regenerate_creates_new_ai_answer_and_keeps_confirmation_buttons(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda approval_item, _approvals, _client: approval_item.update({"resposta_sugerida": "Nova resposta da IA."}) or "Nova resposta da IA.",
    )
    ...
    assert state["question_approval_tokens"]["ABCDEFGH"]["suggested_response"] == "Nova resposta da IA."
...
def test_whatsapp_recognizes_user_guidance_and_regenerates_same_active_question(monkeypatch):
    ...
    assert state["question_approval_tokens"]["ABCDEFGH"]["used"] is False
    assert state["question_approval_tokens"]["ABCDEFGH"]["suggested_response"] == "Sim, e estriada dos dois lados."
...
def test_natural_question_action_accepts_short_confirmation_after_revision():
    assert whatsapp_bridge._question_natural_action("Responda isso") == "approve"
    assert whatsapp_bridge._question_natural_action("Resposta isso") == "approve"
...
def test_natural_approval_answers_active_question_without_requesting_store(monkeypatch):
    approval = {
        "id": "approval-1",
        "status": "pending",
        "loja": "JK Pecas",
    ...
    assert len(calls) == 1
    assert calls[0][0].approval_id == "approval-1"
    assert calls[0][0].resposta == "Sim, e estriada dos dois lados."
    assert state["question_approval_tokens"]["ABCDEFGH"]["used"] is True
    assert state["question_active_threads"] == {}
    assert "RESPOSTA ENVIADA" in replies[-1][0]
...
def test_whatsapp_full_mobile_uses_full_catalog_and_same_number_approval(codex_runtime):
    query = codex_console.codex_criar_tarefa_para_sessao(
        codex_console.CodexTaskRequest(
    ...
    mutation = codex_console.codex_criar_tarefa_para_sessao(
        codex_console.CodexTaskRequest(
            prompt="Sincronize as vendas da loja JK Pecas",
            sandbox="workspace_write",
            approval_mode="request",
        ),
        _full_session(),
    ...
    assert wrong_subject.value.status_code == 404

    approved = codex_console.codex_aprovar_tarefa_para_sessao(
        mutation["task_id"],
        _full_session(),
        codex_console.CodexTaskApprovalRequest(screen_context={"modulo_atual": "black_jhon_mobile"}),
        approval_source="whatsapp",
        subject_id="subject-1",
    )["task"]
    assert approved["status"] == "queued"
    assert approved["sandbox"] == "full_access"
    assert approved["approval_mode"] == "full_access"
    assert approved["scope"]["reason"] == "whatsapp_full_user_approval"
...
def test_whatsapp_approval_requires_explicit_app_scope(codex_runtime, monkeypatch):
    task = codex_console.codex_criar_tarefa_para_sessao(
    ...
    monkeypatch.setattr(codex_console, "_codex_require_full_admin", lambda *_args, **_kwargs: _full_session())
    with pytest.raises(HTTPException) as exc:
        codex_console.codex_aprovar_tarefa(task["task_id"], _request(), None, None)
    assert exc.value.status_code == 400

    approved = codex_console.codex_aprovar_tarefa(
        task["task_id"],
        _request(),
        codex_console.CodexTaskApprovalRequest(screen_context={"modulo_atual": "configuracoes"}),
        None,
    )["task"]
    assert approved["status"] == "queued"
    assert approved["scope"]["enforced"] is True
    assert approved["scope"]["reason"] == "whatsapp_app_approval"
...
def test_whatsapp_approval_code_is_one_time_and_bound_to_subject(codex_runtime, monkeypatch):
    task = codex_console.codex_criar_tarefa_para_sessao(
        codex_console.CodexTaskRequest(prompt="Altere uma configuracao", sandbox="workspace_write", approval_mode="request"),
        _full_session(),
        origin="whatsapp",
    ...
    state = {
        "pending_messages": {
            "origin": {
                ...
                "username": "admin",
                "client_id": "cliente",
                "request_text": "Altere uma configuracao",
    ...
    assert approved["status"] == "queued"
    assert approved["sandbox"] == "full_access"
    assert state["pending_messages"]["origin"]["approval_used"] is True
    assert sent[-1][0] == "approval"

    assert whatsapp_bridge._handle_approval_command(
        {},
        state,
        {"message_id": "replay", "subject_id": "subject-1", "text_body": "APROVAR ABCD2345"},
        _full_session(),
    ) is True
    assert "CÓDIGO INVÁLIDO" in sent[-1][1]["response"]


def test_sales_mobile_action_is_blocked_without_approval_code(monkeypatch):
    proposal = {
    ...
    handled = whatsapp_bridge._try_create_action_pending(
        {},
        state,
        {"message_id": "wamid.action", "subject_id": "subject-1"},
        _full_session(),
    ...
    assert "APROVAR" not in sent[-1][1]["response"]
    assert "SOMENTE CONSULTA" in sent[-1][1]["response"]


def test_stock_mutation_from_whatsapp_is_blocked_even_for_full_user(monkeypatch):
    proposal = {
    ...
    handled = whatsapp_bridge._try_create_action_pending(
        {},
        state,
        {"message_id": "wamid.stock", "subject_id": "subject-1"},
        _full_session(),
        "wa-cliente-admin-subject",
    ...
    assert handled is True
    assert "pending_messages" not in state
...
def test_post_sale_mercado_livre_action_keeps_normal_approval_flow(monkeypatch):
    ...
    monkeypatch.setattr(
        codex_console,
        "codex_criar_tarefa_para_sessao",
        lambda *_args, **_kwargs: pytest.fail("post-sale action should use the approval proposal flow"),
    )
...
def test_bound_full_number_auto_approves_fallback_task_without_code(monkeypatch):
    monkeypatch.setattr(whatsapp_bridge, "_reload_bound_session", lambda *_args: _full_session())
    monkeypatch.setattr(whatsapp_bridge, "_save_config", lambda *_args: None)
    ...
    assert approvals == [("task-write", {"approval_source": "whatsapp", "subject_id": "subject-1"})]
    assert saved[0][1]["trusted_bound_number"] is True
    assert saved[0][1]["awaiting_notified"] is True
    assert "APROVAR" not in sent[0][1]["response"]
...
def test_api_query_with_exact_store_creates_read_only_task(monkeypatch):
    ...
    assert created["payload"].reasoning_effort == "xhigh"
...
def test_protected_whatsapp_mutation_is_blocked_before_task_or_approval(monkeypatch, message_text):
    _configure_whatsapp_process_test(monkeypatch, ["JK Pecas"])
    sent = []
    monkeypatch.setattr(whatsapp_bridge, "_post_command_reply", lambda _config, _message_id, text, title: sent.append((title, text)))
    monkeypatch.setattr(
        codex_console,
        "codex_criar_tarefa_para_sessao",
        lambda *_args, **_kwargs: pytest.fail("protected mutation must not create a task"),
    )
    monkeypatch.setattr(
        whatsapp_bridge,
        "_try_create_action_pending",
        lambda *_args, **_kwargs: pytest.fail("protected mutation must not create a proposal"),
    )
    ...
    assert len(sent) == 1
    assert "SOMENTE CONSULTA" in sent[0][0]
    assert "Nenhum codigo de aprovacao foi criado" in sent[0][1]
    assert "APROVAR" not in sent[0][1]
    assert not state.get("pending_messages")
