...
from backend.services import codex_assistant, codex_readonly_sources
...
def test_visits_is_get_only_store_scoped_and_authoritative_zero(monkeypatch) -> None:
    ...
    result = ml_tools._ia_tool_get_mercado_livre_visits(
...
def test_promotions_is_get_only_and_complete_zero_is_authoritative(monkeypatch) -> None:
    ...
    monkeypatch.setattr(ml_tools, "_ml_api_request", fake_api, raising=False)
    result = ml_tools._ia_tool_get_mercado_livre_promotions(
...
def test_registry_permissions_routing_and_sensitive_cache_contract(monkeypatch) -> None:
    ...
    assert codex_assistant._assistant_tool_meta("mercado_livre_post_sale_detail")["sensitive"] is True
    assert codex_assistant.ASSISTANT_TOOL_PERMISSION_REQUIREMENTS["mercado_livre_visits"] == ("anuncios_ml",)
    assert codex_assistant.ASSISTANT_TOOL_PERMISSION_REQUIREMENTS["mercado_livre_post_sale_detail"] == ("perguntas_pos_venda",)

    visits = codex_assistant._assistant_source_routing_policy("Visitas do anuncio MLB123456789 no Mercado Livre")
    promotions = codex_assistant._assistant_source_routing_policy("Campanhas ativas da loja no Mercado Livre")
    post_sale = codex_assistant._assistant_source_routing_policy("Mensagens da conversa do pack 1234567890 no pos-venda")
    commercial = codex_assistant._assistant_source_routing_policy("Taxas e frete do anuncio MLB123456789")
    ...
    monkeypatch.setattr(codex_assistant, "_assistant_cache_set", lambda *args, **_kwargs: cache_writes.append(args))
    monkeypatch.setattr(codex_assistant, "_assistant_api_query_audit", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(ml_tools, "_ia_ml_resolver_loja_exata", lambda *_args: ("JK Pecas", {}))
    monkeypatch.setattr(
...
def test_codex_executor_forwards_new_readonly_filters(monkeypatch) -> None:
    ...
    visits = codex_assistant.codex_assistant_execute_tool_call(
        "tenant-a",
        "mercado_livre_visits",
        {"mensagem": "visitas", "loja": "JK Pecas", "item_id": "MLB123456789", "dias": 14},
        permissions={"anuncios_ml": True},
    )
    promotions = codex_assistant.codex_assistant_execute_tool_call(
        "tenant-a",
