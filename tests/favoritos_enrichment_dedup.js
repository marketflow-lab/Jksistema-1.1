...
function createHarness() {
    ...
    const context = {
        ...
        fetch: async (_url, options) => {
            ...
            if (state.requestDelayMs > 0) {
                await new Promise(resolve => setTimeout(resolve, state.requestDelayMs));
            }
...
async function run() {
    ...
    await assert.rejects(
        timeoutHarness.context.executarComTimeoutFavoritos(
            () => new Promise(() => {}),
            50
        ),
        error => error && error.favoritosTimeout === true,
        'uma etapa sem resposta deve terminar no prazo local'
    );
    ...
    concurrentHarness.state.requestDelayMs = 35;
    const concurrentExecution = concurrentHarness.context.criarContextoEnriquecimentoFavoritosExecucao();
    const mesmoItemConcorrente = item('MLB-CONCORRENTE');
    await Promise.all([
        concurrentHarness.context.enriquecerAnunciosFavoritosRanking([mesmoItemConcorrente], concurrentExecution),
        concurrentHarness.context.enriquecerAnunciosFavoritosRanking([item('MLB-CONCORRENTE')], concurrentExecution)
    ]);
    assert.strictEqual(concurrentHarness.requests.length, 1, 'mesmo MLB concorrente deve compartilhar a requisicao em andamento');

    const limiterHarness = createHarness();
    limiterHarness.state.requestDelayMs = 35;
    const limiterExecution = limiterHarness.context.criarContextoEnriquecimentoFavoritosExecucao();
    await Promise.all(Array.from({ length: 4 }, (_valor, grupo) => (
        limiterHarness.context.enriquecerAnunciosFavoritosRanking(
            Array.from({ length: 10 }, (_item, index) => item(`MLB-L${grupo}-${index}`)),
            limiterExecution
        )
    )));
    assert.strictEqual(limiterHarness.state.maxActiveRequests, 4, 'pool deve permitir quatro lotes simultaneos de enriquecimento backend');
    ...
    assert.strictEqual(deveEncerrarPlateau({ passada: 2, passadaCompleta: true, assinaturaAtual: assinaturaA, assinaturaAnterior: assinaturaA, pendentesAvant: 0, mutationQuietMs: 799 }), false);
    assert.strictEqual(deveEncerrarPlateau({ passada: 2, passadaCompleta: true, assinaturaAtual: assinaturaA, assinaturaAnterior: assinaturaA, pendentesAvant: 0, mutationQuietMs: 800 }), true);
