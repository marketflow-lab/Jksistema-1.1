...
def test_bling_auth_failure_skips_external_fallback_and_uses_local_stock(monkeypatch):
    from backend.services import codex_bling_tools

    bling_calls: list[str] = []
    local_calls: list[str] = []

    def fake_bling(**kwargs):
        bling_calls.append(str(kwargs.get("tool_id") or ""))
        return {
            "function": "bling_stock_balances",
            "arguments": {},
            "result": {
                "saldos": [],
                "records": 0,
                "warnings": ["Token Bling expirado para esta loja. Refaca a conexao em Integracoes."],
                "read_only": True,
            },
        }

    def fake_local(name: str, *_args, **_kwargs):
        local_calls.append(name)
        return {
            "function": name,
            "arguments": {},
            "result": {"saldos": [{"sku": "001", "saldo": 10}], "read_only": True},
        }
    ...
    result = codex_assistant.codex_assistant_execute_tool_call(
        ...
        permissions={**_permissions(), "estoque": True},
    )
...
def test_complete_zero_listing_is_conclusive_but_partial_zero_is_not():
    complete = {
        ...
        "summary": {"coverage_complete": True, "partial_response": False, "paging": {"has_more": False}},
    }
    partial = {
        **complete,
...
def test_product_fallback_cannot_make_empty_local_stock_sufficient():
    validation = codex_assistant._assistant_tool_validation(
        ...
        warnings=["Fallback automatico apos dados insuficientes: product_data."],
        empty_reasons=["Consulta read-only nao retornou registros."],
        next_fallbacks=[],
        registry_results=[
            {"tool_id": "stock_data", "records": 0, "summary": {"found": False}},
...
def test_real_latest_return_question_forces_one_result_and_forwards_sku(monkeypatch):
    calls: list[tuple[str, tuple[Any, ...], dict[str, Any]]] = []
    ...
    def fake_call(name: str, *args: Any, **kwargs: Any):
        ...
        return {
            ...
            "arguments": {},
            "result": {
                "devolucoes": [{"order_id": "65657", "sku": "200"}],
                "target": {"sku": "200", "item_ids": ["MLB123456789"]},
                "match": {"exact": True, "matched_by": "seller_sku"},
    ...
    name, positional, kwargs = calls[0]
    assert name == "_ia_tool_get_mercado_livre_returns"
    assert positional[:2] == ("tenant", message)
    assert kwargs["sku"] == "200"
    assert kwargs["limite"] == 1
    assert kwargs["offset"] == 0
    assert kwargs["force_refresh"] is True


def test_latest_sale_exposes_exact_order_instead_of_sku_aggregate(monkeypatch):
    def fake_call(_name: str, *args: Any, **kwargs: Any):
        ...
        return {
            ...
            "arguments": {},
            "result": {
                "orders": [{"order_id": "65657", "sku": "200"}],
                "by_sku": [{"sku": "200", "quantity": 1}],
                "target": {"sku": "200", "item_ids": ["MLB123"]},
                "match": {"exact": True, "matched_by": "seller_sku"},
                "exact_coverage": {"complete": True, "stop_reason": "exact_match"},
                "read_only": True,
            },
        }
    ...
    result = codex_assistant.codex_assistant_execute_tool_call(
        ...
        "mercado_livre_orders",
        {
            "message": "E a ultima venda dele. Busque pelo ML. SKU 200 na JK Pecas",
            "loja": "JK Pecas",
            "sku": "200",
            "limite": 100,
        },
...
def test_exact_latest_event_validation_requires_complete_direct_coverage():
    direct_complete = {
        "tool_id": "mercado_livre_returns",
        "records": 1,
        "summary": {
    ...
    partial = codex_assistant._assistant_tool_validation(
        ...
        warnings=["historico local consultado como apoio"],
        empty_reasons=[],
        next_fallbacks=[],
...
def test_exact_target_audit_excludes_personal_data(tmp_path, monkeypatch):
    ...
    result = {
        "records": 1,
        "warnings": [],
        "all_rows": [{"buyer": "NAO_PODE_VAZAR", "message": "privada"}],
        "summary": [
            {
                "tool_id": "mercado_livre_returns",
                "summary": {
                    "target": {"sku": "200", "item_ids": ["MLB123"]},
                    "match": {"exact": True, "matched_by": "variation_seller_sku"},
                    "exact_coverage": {
                        "complete": True,
                        "stop_reason": "matched",
                        "pages_fetched": 2,
    ...
    assert "NAO_PODE_VAZAR" not in raw
    assert "privada" not in raw
