...
def test_firebase_auto_login_reads_only_the_requested_user(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda username: calls.append(username) or remote,
        raising=False,
    )
    monkeypatch.setattr(
        login_core,
        "carregar_usuarios_sheets",
        lambda **_kwargs: pytest.fail("login no modo auto nao deve listar todos os usuarios"),
        raising=False,
    )
...
def test_firebase_auto_login_falls_back_without_streaming_firebase(monkeypatch):
    local = {"usuario": {"username": "usuario", "source": "firebase-cache"}}
    monkeypatch.setattr(login_core, "_firebase_deve_usar", lambda: True, raising=False)
    monkeypatch.setattr(login_core, "_firebase_access_obrigatorio", lambda: False, raising=False)
    monkeypatch.setattr(login_core, "_firebase_obter_usuario", lambda _username: None, raising=False)
    monkeypatch.setattr(
        login_core,
        "_firebase_listar_usuarios",
        lambda **_kwargs: pytest.fail("fallback do login nao pode repetir consulta Firebase"),
        raising=False,
    )
    monkeypatch.setattr(login_core, "_carregar_usuarios_sql", lambda: (local, ["local-header"]), raising=False)

    usuario, worksheet, headers = login_core._carregar_usuario_login("Usuario")

    assert usuario is local["usuario"]
    assert worksheet is None
    assert headers == ["local-header"]
