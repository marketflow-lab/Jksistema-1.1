...
def _manual_capabilities() -> list[dict[str, Any]]:
    ...
    return [
        ...
        _capability(
            cid="contar_skus_com_estoque_bling",
            module="bling",
            category="consultar",
            title="Contar SKUs com estoque na Bling",
            description="Percorre o catalogo da loja e conta SKUs distintos com saldo positivo fora do Full, em modo read-only.",
            intent_examples=["quantos SKUs estao com estoque", "total de produtos com saldo positivo", "numero de SKUs com estoque na loja"],
            input_schema={"loja": "nome exato da loja"},
            read_only=True,
            executor_type="data_tool",
            executor_id="bling_positive_stock_sku_count",
            sources=["Bling v3"],
            status_text="contando SKUs com estoque na Bling",
            fallback_capabilities=[],
            output_summary="Contagem distinta confirmada somente quando catalogo, depositos e saldos tiverem cobertura completa.",
        ),
        _capability(
