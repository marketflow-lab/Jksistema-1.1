...
def generate_scheduled_report_chart_artifacts(
    *,
    ...
    try:
        ...
        requested = max(0, min(int(max_images or 0), REPORT_CHART_MAX_IMAGES))
        if requested <= 0:
            artifacts = []
            discarded_kinds: list[str] = []
        else:
            # Gere os dois candidatos para conseguir escolher o modelo detalhado
            # mesmo quando o primeiro retorno for o antigo cartao simples de KPI.
            raw = report_charts.generate_report_charts(
                safe_data,
                output_dir,
                max_images=REPORT_CHART_MAX_IMAGES,
            )
            generated = _validated_artifacts(raw, output_dir, REPORT_CHART_MAX_IMAGES)
            detailed = [item for item in generated if str(item.get("kind") or "") != "kpi_card"]
            artifacts = detailed[:1]
            kept_paths = {str(item.get("path") or "") for item in artifacts}
            discarded_kinds = []
            for item in generated:
                if str(item.get("path") or "") in kept_paths:
                    continue
                discarded_kinds.append(str(item.get("kind") or "unknown"))
                try:
        ...
        return {
            ...
            "status": "generated" if artifacts else "generation_empty",
            "artifacts": artifacts,
            "discarded_kinds": discarded_kinds,
            "report_kind": "monthly" if _text_key(kind) in {"monthly", "mensal", "mes"} else "weekly",
