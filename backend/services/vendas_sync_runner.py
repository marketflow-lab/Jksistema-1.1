...
from backend.services.vendas_sync_periodo import _sincronizar_vendas_periodo_impl
...
async def sincronizar_vendas(req: VendasSyncRequest, client_id: str = Depends(vendas_context.get_tenant_id)):
...
def configure_vendas_sync_runner_runtime(runtime_module=None):
    return _configure_runtime_globals(runtime_module)


configure_vendas_sync_runner_runtime()
