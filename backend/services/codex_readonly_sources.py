    ...
    **_: Any,
    ...
    for store in requested:
        ...
        try:
            ...
            checked.append(name)
            questions_by_store[name] = len(questions)
            ...
            if bool(payload.get("interrompido")):
                ...
                warnings.append(f"{name}: a API interrompeu a varredura antes de confirmar toda a fila.")
        except Exception as exc:
            detail = getattr(exc, "detail", exc)
    ...
    result.update({
        ...
        "total_pending": len(records) if status_filter == "UNANSWERED" else None,
