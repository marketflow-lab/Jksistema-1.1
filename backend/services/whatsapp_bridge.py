...
def _whatsapp_ranking_items(lines: list[str]) -> list[dict[str, Any]]:
    ...
    def append_current(*, discard_orphan: bool = False) -> None:
        ...
        if not current:
            return
        # Textos introdutorios ou totais gerais anteriores ao primeiro SKU nao
        # podem virar um cartao falso "SKU nao informado". Quando um SKU real
        # aparece, descarte esse bloco orfao; se nao houver SKU algum, o bloco
        # ainda e preservado como fallback ao final da leitura.
    ...
    for line in lines:
        ...
        # Relatorios redigidos pela IA costumam numerar o ranking como
        # "1. SKU 008" (frequentemente em negrito), sem usar "SKU:". Esse
        # formato precisa abrir um novo item antes de lermos quantidade e valor;
        # caso contrario todos os produtos acabam agrupados e so os totais do
        # ultimo SKU permanecem no cartao.
