...
from .dependencies import get_tenant_path
from .errors import VendasDomainError
from .state import SYNC_STATE_LOCK
...
def _vendas_sync_parse_date(valor: str, campo: str) -> dt.date:
    try:
        ...
        raise VendasDomainError(status_code=400, detail=f"Data invalida em {campo}. Use YYYY-MM-DD.")
...
def _vendas_sync_dias_periodo(data_inicio: str, data_fim: str) -> list[str]:
    ...
    if inicio > fim:
        raise VendasDomainError(status_code=400, detail="Data inicial nao pode ser maior que a data final.")
