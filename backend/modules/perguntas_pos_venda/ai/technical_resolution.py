...
def technical_question_plan_prompt(context: Mapping[str, Any]) -> str:
    prompt = (
        ...
        "dados pessoais ou identificadores do comprador em consultas. Quando uma caracteristica de peca automotiva "
        "nao estiver no anuncio ou na referencia exata do SKU, planeje a consulta da especificacao da peca original "
        "do veiculo correspondente, incluindo codigo OEM, modelo e variacao disponiveis. Pesquise o atributo de "
        "forma neutra, sem pressupor como correta uma cor, medida ou propriedade sugerida na pergunta. "
        "Planeje separadamente a comprovacao do vinculo entre SKU/variacao e peca/referencia vendida e a "
        "comprovacao da aplicacao dessa referencia no alvo. A API oficial informa o estado do anuncio, mas "
        "atributos tecnicos do vendedor exigem confronto com o SKU e fontes tecnicas. Se houver codigos "
        "misturados ou contradicao no anuncio, investigue componente, posicao, lado, cor e variacao de cada "
        "referencia. Fotos e respostas anteriores da loja nao bastam como prova. Se os vinculos ja estiverem "
        "comprovados, nao abra lacuna de codigo original quando faltar somente geracao ou ano/carroceria "
        "do comprador; aproveite esses dados quando ja constarem do historico. "
        "Quando o alvo de compatibilidade for uma familia, modelo, serie ou ano sem variacao exata, crie requisitos essenciais para: "
        "identificar o mercado e periodo; escolher apenas os eixos de variacao que mudam a aplicacao segundo a funcao do produto; "
        "enumerar o universo completo de versoes; e mapear a referencia exata do produto contra cada versao ou grupo homogeneo. "
        "Em veiculos, considere geracao, motor, combustivel, carroceria, cambio ou tracao somente quando pertinentes; em eletronicos, "
        "revisao, tensao, conector, protocolo ou regiao; em maquinas, serie, motor, eixo, rosca ou fixacao; nos demais alvos, medidas, "
        "montagem, interface e codigos. Inclua uma query target_variant_universe_official para o universo do alvo e outra para a cobertura exata do "
        "produto. Nao presuma que citar algumas versoes representa todas. "
        "Responda exclusivamente em JSON no contrato "
...
def technical_resolution_prompt(
    ...
    *,
    ...
    return (
        ...
        "de versoes estiver completo e a referencia exata do produto cobrir 100% das versoes, sem excecao, conflito ou requisito pendente. "
        "Se a cobertura atingir apenas um subconjunto, decida conditional e identifique o unico discriminador decisivo que separa as versoes cobertas. Se o universo ou o mapeamento continuar incompleto, decida insufficient e gere gap_queries especificas para as versoes ou eixos pendentes. "
        "Nao use somente a lista parcial do anuncio como denominador nem trate engine/motor como eixo obrigatorio quando outro atributo alterar a aplicacao. Nao invente fatos. O aplicativo preservara sua decisao factual e o commercial_state que voce adjudicar; validara somente "
