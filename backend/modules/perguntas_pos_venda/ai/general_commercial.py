...
from .runtime import AIAnswer, _perguntas_ia_assinatura_loja, resolve_runtime_adapter


_GENERAL_FIT_CATEGORIES = frozenset({"general", "product_feature", "unknown"})
...
def _general_research_final_prompt(
    ...
    *,
    ...
    state_rules = (
        ...
        if regulated else
        "Respeite o estado comercial apurado no bloco AVALIACAO_TECNICA_INTERNA. Em fits, use CTA para o produto atual; "
    ...
    return (
        prompt + "\n\nETAPA DE PESQUISA EXTERNA OBRIGATORIA: preserve o rascunho existente e use somente as afirmacoes "
        "verified devolvidas pelo dossie para complementar fatos tecnicos pertinentes a pergunta. Responda diretamente compatibilidade, "
        ...
        "de produto, codigo OEM/referencia, medida, aplicacao ou caracteristica. A ativacao por fonte oficial ou duas fontes "
        "tecnicas independentes ja foi decidida fora do modelo; nunca promova candidatos por conta propria. "
        "Anuncios similares sao apenas pistas e nao aparecem como fatos verified. Dados autenticados da loja, "
        ...
        "Se o dossie nao trouxer o fato verified necessario, mantenha a conclusao inconclusiva. "
        + commercial_rules
        + state_rules
        ...
        + _untrusted_compact_block("dossie_tecnico_verificado", result, 10000)
        + "\n\nAVALIACAO_TECNICA_INTERNA:\n"
        + _untrusted_compact_block("avaliacao_tecnica_interna", fit_assessment, 7000)
...
def _general_fit_evaluation_prompt(
    ...
    return (
        ...
        "Avalie todas as subperguntas essenciais e classifique commercial_state como fits, variant, partial, insufficient, "
        ...
        "somente quando uma incompatibilidade objetiva estiver comprovada. Resultado vazio de pesquisa nunca prova incompatibilidade. "
        ...
        "maximo tres frases de conteudo e a assinatura literal em paragrafo separado, para ser preservado caso a geracao final falhe. "
...
def _evaluate_general_fit_and_alternative(
    ...
    *,
    ...
    if regulated or not (categories & _GENERAL_FIT_CATEGORIES):
        state = "not_applicable"
        client.commercial_state = state
        client.context_pipeline.extend([
            {"step": 5, "name": "commercial_fit_evaluation", "status": "disabled" if regulated else "not_applicable",
             "commercial_state": state},
            {"step": 6, "name": "same_store_technically_verified_alternative", "status": "disabled" if regulated else "skipped",
             "alternative_used": False},
        ])
        return None, _general_fit_assessment_payload(client, None), alternative

    assessment = client._call_model(
        _general_fit_evaluation_prompt(client, metadata, hub, result, internal_sources),
        {**metadata, "category": "compatibility", "source_category": metadata.get("category")},
        stage="commercial_fit_evaluation",
        tool_results=[*(internal_sources or []), hub, result],
    )
    state = str(getattr(client, "commercial_state", "") or "").strip().lower()
    raw_analysis = getattr(client, "compatibility_analysis", {})
    analysis = raw_analysis if isinstance(raw_analysis, dict) else {}
    decision = str(analysis.get("decision") or "insufficient").strip().lower()
    allowed_states = {"fits", "variant", "partial", "insufficient", "incompatible", "not_applicable"}
    if decision == "insufficient":
        ...
        state = "incompatible"
    elif decision == "yes" and (state not in allowed_states or state == "incompatible"):
        state = "fits"
    elif decision != "yes":
        state = "insufficient"
    client.commercial_state = state
    client.context_pipeline.append({
        "step": 5, "name": "commercial_fit_evaluation",
        "status": "completed" if str(getattr(assessment, "answer", "") or "").strip() else "empty",
        "commercial_state": state,
    })
    ...
    return assessment, _general_fit_assessment_payload(client, assessment), alternative


def _append_general_final_pipeline(client, behavior_profile: dict, *, regulated: bool) -> None:
    client.context_pipeline.extend([
        {"step": 7, "name": "seller_behavior_profile_v2", "status": "disabled" if regulated else "completed",
         "profile_applied": bool(not regulated and behavior_profile.get("profile_active"))},
        {"step": 8, "name": "regulated_factual_generation" if regulated else "commercial_final_generation", "status": "completed"},
    ])
