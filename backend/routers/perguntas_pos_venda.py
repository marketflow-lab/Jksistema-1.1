...
LEGACY_PERGUNTAS_POS_VENDA_ROUTES: tuple[LegacyRouteSpec, ...] = (
    ...
    LegacyRouteSpec("PATCH", "/api/mercadolivre/assistant/jobs/{job_id}/draft", "ml_customer_reply_job_draft_patch"),
    LegacyRouteSpec("GET", "/api/mercadolivre/assistant/solicitacoes", "ml_customer_reply_solicitacoes_list"),
...
TRAINING_READ_ROUTES: tuple[LegacyRouteSpec, ...] = (
    LegacyRouteSpec("GET", "/api/mercadolivre/ia-treinamento/ficha", "ml_ia_treinamento_ficha_obter"),
    LegacyRouteSpec("POST", "/api/mercadolivre/ia-treinamento/ficha/atualizar", "ml_ia_treinamento_ficha_atualizar"),
)
...
def create_perguntas_pos_venda_router() -> APIRouter:
    ...
    for spec in (*LEGACY_PERGUNTAS_POS_VENDA_ROUTES, *PERGUNTAS_LOADING_ROUTES, *TRAINING_READ_ROUTES):
