...
describe("public gateway routes", () => {
  ...
  it("publishes the backward-compatible gateway protocol and build version", async () => {
    const response = await worker.fetch(
  ...
  it("enforces at most three images for one inbound response", async () => {
    const limited = outboundImageEnvironment({ reserve: false, existing: null, messageImageCount: 3 });
    ...
    expect(await response.json()).toMatchObject({ success: false, error: "outbound_image_response_limit" });
  ...
  it("rejects an altered proactive chart before reserving outbound quota", async () => {
    ...
    expect(response.status).toBe(400);
    expect(await response.json()).toMatchObject({ success: false, error: "outbound_image_sha256_mismatch" });
    expect(sqlCalls.some((item) => item.sql.includes("INSERT OR IGNORE INTO outbound_media"))).toBe(false);
    expect(graphRequests).toHaveLength(0);
  });
