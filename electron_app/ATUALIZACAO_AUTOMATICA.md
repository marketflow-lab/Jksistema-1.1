...
Toda versao publica disponibiliza um `Setup` completo para instalacoes novas e recuperacao. No modo `app-update`, a mesma release tambem publica o `Update` leve usado pelo atualizador automatico.
...
3. Abra o workflow **Desktop release** e informe a tag correspondente, por exemplo `v1.0.154`.
4. Escolha `app-update` para publicar o `Update` leve junto do `Setup` completo da mesma versao. Escolha `runtime-update` quando o runtime mudar e o `Setup` completo precisar ser o pacote usado pelo atualizador.
...
O workflow nao roda em `push` e nao publica sem a opcao explicita. A tag informada deve coincidir com a versao de `electron_app/package.json`. No modo `app-update`, o staging aceita exatamente os executaveis `Setup` e `Update`, seus blockmaps e somente o `latest.yml` gerado pelo `Update`.
...
- `latest.yml` deve vir de somente um perfil por release. `app-update` usa o manifesto do `Update` leve; `runtime-update` usa o manifesto do `Setup` completo.
- Downloads destinados a instalacao manual devem apontar para `JK-Sistema-Cliente-Setup-<versao>.exe`. O executavel `Update` fica reservado ao atualizador automatico e a computadores com uma instalacao reconhecida.
