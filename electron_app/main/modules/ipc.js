...
app.whenReady().then(async () => {
    ...
    ipcMain.handle('favoritos-worker:pause', async (event) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:pause');
        return pauseFavoritosWorkerBrowser();
    });
    ipcMain.handle('favoritos-worker:resume', async (event) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:resume');
        return resumeFavoritosWorkerBrowser();
    });
    ipcMain.handle('favoritos-worker:cancel', async (event) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:cancel');
        return cancelFavoritosWorkerBrowser();
    });
    ipcMain.handle('favoritos-worker:status', async (event) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:status');
        return favoritosWorkerBrowserStatus();
    });
    ipcMain.handle('favoritos-worker:show', async (event) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:show');
        return showFavoritosWorkerBrowser();
    });
    ipcMain.handle('favoritos-worker:hide', async (event) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:hide');
        return hideFavoritosWorkerBrowser();
    });
    ipcMain.handle('favoritos-worker:stop', async (event, options = {}) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:stop');
        return stopFavoritosWorkerBrowser(options || {});
    });
    ipcMain.handle('favoritos-worker:execute', async (event, code) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:execute');
        return executeFavoritosWorkerBrowser(code);
    });
    ipcMain.handle('favoritos-worker:click', async (event, point) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:click');
        return clickFavoritosWorkerBrowser(point || {});
    });
    ipcMain.handle('favoritos-worker:type', async (event, payload) => {
        assertTrustedFavoritosIpcSender(event, 'favoritos-worker:type');
        return typeFavoritosWorkerBrowser(payload || {});
    });
