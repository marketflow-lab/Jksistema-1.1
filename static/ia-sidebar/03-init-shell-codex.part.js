  function init() {
    ...
    function _codexStatusTexto(task) {
      ...
      if (status === 'awaiting_approval') return 'Aguardando aprovacao da proposta.';
      if (status === 'awaiting_input') return 'Aguardando os dados solicitados.';
      ...
      if (status === 'partial') return 'Concluido parcialmente; confira o que nao foi comprovado.';
    ...
    function _codexRenderRuntime(task) {
      ...
      const planText = steps.length
        ? steps.map((step, index) => {
          const state = String(step && (step.state || step.status) || 'pendente');
          const label = String(step && (step.title || step.label || step.name) || `Etapa ${index + 1}`);
          return `${index + 1}. [${state}] ${label}`;
        }).join('\n')
        : String(task.live_plan || '');
      _codexSetLiveBlock('jk-codex-reasoning', 'Plano e raciocinio', planText || task.reasoning_summary || '');
      const status = String(task.status || '');
      const requiredInput = task.required_input && typeof task.required_input === 'object'
        ? (task.required_input.message || task.required_input.prompt || '')
        : String(task.required_input || '');
      _codexSetLiveBlock(
        'jk-codex-live-answer',
        status === 'awaiting_input' ? 'Dados necessarios' : 'Resposta parcial',
        status === 'completed' ? '' : (requiredInput || task.live_answer || '')
      );
    ...
    function _codexCompactTaskForMemory(task) {
      ...
      if (['completed', 'partial', 'failed', 'canceled'].includes(String(compact.status || ''))) {
    ...
    async function _codexCarregarPropostasPendentes() {
      if (!_usuarioLocalEhFull()) return [];
      try {
        const data = await _codexFetchJson('/api/admin/codex/actions/proposals?status=awaiting_approval&limit=50');
        const proposals = Array.isArray(data && data.proposals) ? data.proposals : [];
        proposals.slice().reverse().forEach(proposal => _codexRenderActionProposal(proposal));
        return proposals;
      } catch (_) {
        return [];
      }
    ...
    function _codexMostrarAprovacao(task) {
      ...
      if (!box) return;
      const ativo = _usuarioLocalEhFull() && !!(task && task.status === 'awaiting_approval' && !task.proposal);
    ...
    function _codexAplicarTask(task) {
      ...
      if (task.proposal && task.proposal.proposal_id) {
        _codexRenderActionProposal(task.proposal);
      }
      const status = String(task.status || '');
      if (status === 'awaiting_approval') {
        _codexSetStatus(task.proposal ? 'Revise e confirme esta acao separadamente.' : 'Aguardando confirmacao de acesso mutavel.');
      } else if (status === 'awaiting_input') {
        const required = task.required_input && typeof task.required_input === 'object'
          ? (task.required_input.message || task.required_input.prompt || '')
          : String(task.required_input || '');
        const message = required || task.final_response || 'Preciso de mais dados para continuar.';
        _codexSetStatus('Aguardando os dados solicitados.');
        if (!codexRenderedFinalTasks.has(task.task_id)) {
          codexRenderedFinalTasks.add(task.task_id);
          _codexAddMsg('assistant', message, '', { task_id: task.task_id });
        }
        ...
        if (!codexRenderedFinalTasks.has(task.task_id)) {
          codexRenderedFinalTasks.add(task.task_id);
          _codexAddMsg('assistant', task.final_response || (status === 'partial' ? 'A tarefa terminou parcialmente, sem resposta final comprovada.' : 'Codex concluiu sem resposta final.'), '', { task_id: task.task_id });
        }
    ...
    function _codexRenderActionProposal(proposal) {
      ...
      const meta = document.createElement('div');
      meta.className = 'jk-codex-action-meta';
      meta.textContent = `${action.module || 'sistema'} | ${proposal.action_id || action.id || '-'} | versao ${proposal.version || 1} | ${proposal.risk || action.risk_level || 'local_write'}`
      ...
      safeguards.className = 'jk-codex-action-detail';
      const preconditions = Array.isArray(proposal.preconditions) ? proposal.preconditions : [];
      const postconditions = Array.isArray(proposal.postconditions) ? proposal.postconditions : [];
      const channels = Array.isArray(proposal.channels_allowed) ? proposal.channels_allowed : [];
      safeguards.textContent = [
        preconditions.length ? `Antes: ${preconditions.join('; ')}` : '',
        postconditions.length ? `Verificacao: ${postconditions.join('; ')}` : '',
        channels.length ? `Confirmacao permitida: ${channels.join(', ')}` : '',
        proposal.requires_app_confirmation ? 'Esta acao so pode ser confirmada no aplicativo.' : '',
      ].filter(Boolean).join('\n');
      ...
      approve.addEventListener('click', () => _codexAprovarActionProposal(proposal, card));
      const cancel = document.createElement('button');
      cancel.type = 'button';
      cancel.className = 'jk-codex-btn danger';
      cancel.textContent = 'Cancelar';
      cancel.addEventListener('click', async () => {
        ...
        try {
          await _codexFetchJson('/api/admin/codex/actions/proposals/' + encodeURIComponent(proposal.proposal_id) + '/reject', {
            method: 'POST',
          });
          card.classList.add('is-canceled');
          _codexSetStatus('Acao rejeitada antes da execucao.');
        } catch (err) {
          approve.disabled = false;
          cancel.disabled = false;
          _codexSetStatus(_codexErroCurto(err, 'Falha ao rejeitar acao.'), true);
        }
      });
      ...
      if (safeguards.textContent) card.appendChild(safeguards);
    ...
    async function _codexAprovarActionProposal(proposal, card) {
      if (!_usuarioLocalEhFull()) return;
      const id = String(proposal && proposal.proposal_id || '').trim();
      if (!id) return;
      if (card) {
      ...
      try {
        const data = await _codexFetchJson('/api/admin/codex/actions/proposals/' + encodeURIComponent(id) + '/approve', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            proposal_version: Number(proposal.version || 1),
    ...
    async function _codexPollActionRun(runId) {
      ...
      try {
        ...
        if (['queued', 'running', 'cancel_requested'].includes(status)) {
          ...
          _codexSetStatus('Acao executada, mas a verificacao ficou parcial.', true);
        } else if (status === 'failed') {
    ...
    async function _codexCriarTarefa(forcedAccess = '') {
      ...
      if (input) input.value = '';
      if (full && _blackJhonPrepararAcaoLocalFavoritos(prompt)) {
        return;
      }
      ...
      _codexSetStatus('Enviando solicitacao ao agente Black Jhon...');
      const taskPollGeneration = _codexInvalidarPollAtual();
      try {
        let requestId = '';
        try { requestId = String(globalThis.crypto?.randomUUID?.() || ''); } catch (_) {}
        if (!requestId) requestId = `app_${Date.now()}_${Math.random().toString(36).slice(2, 12)}`;
